...
const STATS = [
  { value: 8, suffix: "", label: "Art collections" },
...
function Counter({ to, suffix }: { to: number; suffix: string }) {
  ...
  return (
    <span ref={ref} className="font-serif text-[clamp(3rem,8vw,6.5rem)] leading-none">
      {val}
      {suffix}
    </span>
  );
...
export default function Numbers() {
  return (
    <section className="border-y border-line bg-surface">
      <div className="container grid grid-cols-2 gap-x-8 gap-y-14 py-20 md:py-28 lg:grid-cols-4">
        {STATS.map((s) => (
