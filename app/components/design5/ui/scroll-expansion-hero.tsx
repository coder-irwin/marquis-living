...
const ScrollExpandMedia = ({
  ...
  // ----- Inset/rounding driven by REAL page scroll (not the curtain reveal) -----
  // While the hero is locked the page can't scroll, so this stays at 0 (full-bleed).
  // Once it unlocks and the user actually scrolls, the video eases into a rounded,
  // margined card.
  const { scrollYProgress: pageScroll } = useScroll({
    target: stageRef,
    offset: ['start start', 'end start'],
  });
  const pageSmooth = useSpring(pageScroll, { stiffness: 120, damping: 30, mass: 0.3 });
  const mediaInset = useTransform(pageSmooth, [0, 0.3], [0, 28]);
  const mediaRadius = useTransform(pageSmooth, [0, 0.3], [0, 28]);
  ...
  return (
    <div ref={sectionRef} className='overflow-x-hidden'>
      <section
        ref={stageRef}
        className='relative flex min-h-[100dvh] w-full flex-col items-center justify-start bg-white'
      >
        {/* ---- Video, full-bleed → insets into a rounded card on scroll ---- */}
        <motion.div
          className='absolute overflow-hidden'
          style={{ top: mediaInset, left: mediaInset, right: mediaInset, bottom: mediaInset, borderRadius: mediaRadius }}
        >
          {isYouTube ? (
            <iframe
        ...
        <motion.div
          className='absolute z-10 bg-black/80'
          style={{
            y: curtainY,
            top: mediaInset,
            left: mediaInset,
